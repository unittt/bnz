...
function CAttrSliderBox.SetInfo(self, datalist)	
	...
	if self.m_BlueSlider <= 0 then
		self.m_WashBtn:SetSpriteName("h7_an_5")
		self.m_WashBtn:SetGrey(true)
		self.m_WashBtn:SetText("[5c6163]洗 点[-]")
	else
		self.m_WashBtn:SetGrey(false)
		self.m_WashBtn:SetSpriteName("h7_an_2")
		self.m_WashBtn:SetText("洗 点")
	end
end
...
function CAttrSliderBox.RefreshSliderRound(self, remainpoint)
	...
	if self.m_RemainPoint <= 0 then
		self.m_AddBtn:SetSpriteName("h7_xiaoan_55_1")
	else
		self.m_AddBtn:SetSpriteName("h7_xiaoan_55")
	end 
	if iPoint == self.m_NowPoint then
		self.m_ReduceBtn:SetSpriteName("h7_xiaoan_56_1")
	else
		self.m_ReduceBtn:SetSpriteName("h7_xiaoan_56")
	end
end
...
function CAttrSliderBox.WashPoint(self)
	...
	local item = DataTools.GetItemData(10004)
	if iItemNum <= 0 then
		-- g_NotifyCtrl:FloatMsg("#G" .. item.name .. "不足")
		local items = {{sid = 10004, count = 0, amount = 1}}
		g_QuickGetCtrl:CurrLackItemInfo(items, {}, nil, function()
			netplayer.C2GSWashPoint(self.m_Keylist[self.m_Index], 1)
		end)
		return
	end
