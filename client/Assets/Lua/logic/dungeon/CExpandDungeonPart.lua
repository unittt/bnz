local CExpandDungeonPart = class("CExpandDungeonPart", CPageBase)

function CExpandDungeonPart.ctor(self, obj)
    CPageBase.ctor(self, obj)
end

function CExpandDungeonPart.OnInitPage(self)
...
function CExpandDungeonPart.RefreshTask(self)
    local oCtrl = g_DungeonTaskCtrl
    local bShow = oCtrl:IsInCommonFuben()
    if bShow then
        self.m_TitleL:SetRichText(oCtrl:GetFubenName(), nil, nil, true)
        -- self.m_GoalL:SetText(oCtrl:GetFubenGoalDesc())
        local bFight = oCtrl:IsFightTask(oCtrl.m_CurFubenTask)
        if bFight then
            self.m_BossDescL:SetRichText("#G"..oCtrl:GetFubenBossDesc(), nil, nil, true)
            self:SetBossSpr(oCtrl:GetFubenBossFigureId())
            self:RefreshRewards(oCtrl:GetFubenRewards())
            self.m_FightDescL:SetRichText(oCtrl:GetFubenDesc(), nil, nil, true)
        else
            self.m_DescL:SetRichText(oCtrl:GetFubenDesc(), nil, nil, true)
        end
...
function CExpandDungeonPart.RefreshRewards(self, rewards)
...
function CExpandDungeonPart.SetBossSpr(self, iFigure)
...
function CExpandDungeonPart.GetRewardBox(self, idx)
...
function CExpandDungeonPart.ResetBgSize(self, bFight)
...
function CExpandDungeonPart.GetWidgetBot(self, oWidget)
    local pos = oWidget:GetLocalPos()
    local iHeight = oWidget.m_UIWidget.height
    return pos.y - iHeight
end

function CExpandDungeonPart.GetBgTop(self)
    local bgPos = self.m_BgWdt:GetLocalPos()
    return bgPos.y
end

function CExpandDungeonPart.OnClickTask(self)
    if g_WarCtrl:IsWar() then return end
    if g_DungeonTaskCtrl:IsInCommonFuben() then
        CTaskHelp.ClickTaskLogic(g_DungeonTaskCtrl.m_CurFubenTask)
    end
end

function CExpandDungeonPart.OnClickItemBox(self, oBox)
    if oBox.itemId then
        g_WindowTipCtrl:SetWindowItemTip(oBox.itemId, {widget=oBox})
    end
end

function CExpandDungeonPart.OnDungeonTaskEvent(self, oCtrl)
    if oCtrl.m_EventID == define.Dungeon.Event.ReceiveFubenTask then
        self:RefreshTask()
    end
end

return CExpandDungeonPart