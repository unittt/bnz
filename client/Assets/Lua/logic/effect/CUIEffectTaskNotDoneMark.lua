local CUIEffectTaskNotDoneMark = class("CUIEffectTaskNotDoneMark", CObject)

function CUIEffectTaskNotDoneMark.ctor(self, oAttach, size, pos)
	...
	g_ResCtrl:LoadCloneAsync("Effect/UI/ui_eff_0024/Prefabs/ui_eff_0024.prefab", callback(self, "OnEffLoad"), false)
end 

function CUIEffectTaskNotDoneMark.OnEffLoad(self, oClone, sPath)
...
function CUIEffectTaskNotDoneMark.RecaluatePanelDepth(self)
	if self.m_Eff then
		self.m_Eff.m_RenderQComponent:RecaluatePanelDepth()
	end
end

return CUIEffectTaskNotDoneMark