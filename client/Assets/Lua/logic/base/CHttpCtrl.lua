...
function CHttpCtrl.PostBytes(self, url, cb, headers, bytes, tArgs)
	tArgs = tArgs or {}
	local timeout = tArgs.timeout or 30
	headers = headers or {}
	local www = self.m_Instance:PostBytes(url, headers, bytes, callback(self, "OnPostResult"))
	self.m_Requests[www] = {cb= cb}
	local timer = Utils.AddTimer(callback(self, "AblortTimeup"), 0, timeout)
	self.m_Requests[www] = {cb= cb, timer = timer, json_result =tArgs.json_result}
	table.print({url=url,headers=headers},"http post ->")
...
function CHttpCtrl.PostHttp(self, url, key1, param1, key2, param2)
	self.m_Instance:PostHttp(url, key1, param1, key2, param2, callback(self, "OnPostResult"))
end
	...
	if info then
		if info.cb then
			...
			if success then
				...
				table.print(tResult, "CHttpCtrl.CommonProcess: cb tResult->")
			end
