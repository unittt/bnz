...
function CEffect.ctor(self, path, layer, cached, cb)
	...
	g_ResCtrl:LoadCloneAsync(path, callback(self, "OnEffLoad"))
...
function CEffect.SetAttachUI(self, oAttach)
	...
	self:SetParent(UITools.GetUIRoot().transform)
...
function CEffect.OnEffLoad(self, oClone, sPath)
	...
	if oClone then
		...
		if self.m_Layer then
			CObject.SetLayer(self, self.m_Layer, true)
		end
		self:ProcessUI()
		self:ProcessTiling()
...
function CEffect.SetLoop(self, b)
	local particle = self:GetMissingComponent(classtype.ParticleAndAnimation)
	if particle then
		particle:PlaySelfAndAllChildren(self.m_GameObject, b)
	end
end
return CEffect
