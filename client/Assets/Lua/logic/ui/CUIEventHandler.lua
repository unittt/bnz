...
function CUIEventHandler.ctor(self, obj)
	...
	self.m_ClkDelta = 0 -- 限制点击频率
end

function CUIEventHandler.Destroy(self)
	if self.m_UIEventHandler then
		for k,_ in pairs(self.m_CallbackDict) do
			self:DelUIEvent(k)
		end
		self.m_EventGo:Destroy()
		self.m_UIEventHandler:Destroy()
	end
end
...
function CUIEventHandler.PlayUISound(self, iEvent)
	if iEvent == enum.UIEvent["click"] then
		if self.m_ClickSoundPath == define.Audio.SoundPath.Tab then
			local curgroup = self.m_UIToggle and self.m_UIToggle.group or nil
			local curid = self:GetInstanceID()
			if curgroup then
				if curid == g_AudioCtrl.m_RecordInfo[curgroup] then
					return
				end
				g_AudioCtrl.m_RecordInfo[curgroup] = curid
			end
		end
...
function CUIEventHandler.AddUIEventHandler(self)
	if not self.m_UIEventHandler then
		if gameconfig.Debug.ClientDebug then
			self.m_UIEventHandler = Utils.GetGameObjComponent(self.m_EventGo, classtype.UIEventHandler)
		else
			self.m_UIEventHandler = self:GetMissingComponent(classtype.UIEventHandler)
		end
	end
...
function CUIEventHandler.DelUIEvent(self, sEvent)
	local iEvent = enum.UIEvent[sEvent]
	self.m_CallbackDict[sEvent] = nil
	if self.m_UIEventHandler then
	...
	if iEvent == enum.UIEvent["click"] then
		...
		if self.m_CallbackDict[enum.UIEvent["doubleclick"]] then
			if not self.m_DoubleClickTimer then
				self.m_DoubleClickTimer = Utils.AddTimer(callback(self, "DoubleClickFail"), 0, 0.2)
		...
		if self.m_ClkTimer then
			return true
		elseif self.m_ClkDelta and self.m_ClkDelta > 0 then
			self.m_ClkTimer = Utils.AddTimer(callback(self, "UnlockClk"), 0, self.m_ClkDelta)
		end
	elseif iEvent == enum.UIEvent["doubleclick"] then
...
function CUIEventHandler.DoubleClickFail(self)
	local iEvent = enum.UIEvent["click"]
	local func = self.m_CallbackDict[iEvent]
	if func then
		self:PlayUISound(iEvent)
...
function CUIEventHandler.UnlockClk(self)
	if self.m_ClkTimer then
		Utils.DelTimer(self.m_ClkTimer)
		self.m_ClkTimer = nil
	end
	return false
end
