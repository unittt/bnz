local CTeamApplyPart = class("CTeamApplyPart", CPageBase)

function CTeamApplyPart.ctor(self, obj)
	CPageBase.ctor(self, obj)
end

function CTeamApplyPart.OnInitPage(self)
...
function CTeamApplyPart.InitContent(self)
...
function CTeamApplyPart.OnCtrlEvent(self, oCtrl)
...
function CTeamApplyPart.UpdateApplyGrid(self)
...
function CTeamApplyPart.GetApplyIndexById(self, iPid)
	local index
...
function CTeamApplyPart.RefreshApply(self)
	-- printc("refresh")
...
function CTeamApplyPart.AddApplyBox(self, dApply)
...
function CTeamApplyPart.DelApplyBox(self, pid)
...
function CTeamApplyPart.RefreshCurTeam(self)
	local iMaxCount = 5
...
function CTeamApplyPart.RequestClearAll(self)
	netteam.C2GSClearApply()
end

function CTeamApplyPart.ClearAndCloseView(self)
	self.m_ApplyGrid:Clear()
	self.m_CurIndex = 0
	self.m_ApplyList = {}
	-- self:CloseView()
...
function CTeamApplyPart.IsExist(self, iPid)
	for i,dApply in ipairs(self.m_ApplyList) do
		if dApply.pid == iPid then
			return true
		end
