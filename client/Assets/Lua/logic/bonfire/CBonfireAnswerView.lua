local CBonfireAnswerView = class("CBonfireAnswerView", CViewBase)

function CBonfireAnswerView.ctor(self, cb)
	CViewBase.ctor(self, "UI/bonfire/BonfireAnswerView.prefab", cb)
	self.m_GroupName = "sub"
	self.m_ExtendClose = "ClickOut"
	self.m_BehindStrike = true
end

function CBonfireAnswerView.OnCreateView(self)
...
function CBonfireAnswerView.SetNoStart(self)
...
function CBonfireAnswerView.SetInfo(self, info)
	if info == nil or next(info) == nil then
		...
		g_BonfireCtrl.m_CurTopicTime = info.time
		g_BonfireCtrl.m_CurQuestionState = 1
	end
	local info = g_BonfireCtrl.m_CurTopicInfo
	self.m_TopicNumber:SetText(info.cur_round.."/"..info.total_round)
	self.m_TopicLabel:SetText(data.bonfiredata.TOPIC[info.id])
	if next(info.choices) then
		self.m_AnswerA:SetText(info.choices[1])
		self.m_AnswerB:SetText(info.choices[2])
		self.m_AnswerC:SetText(info.choices[3])
		self.m_AnswerD:SetText(info.choices[4])
		self.m_AnswerA:NoShowIcon()
		self.m_AnswerB:NoShowIcon()
		self.m_AnswerC:NoShowIcon()
		self.m_AnswerD:NoShowIcon()
	...
	local function update()
		if self.m_TopicTime:IsDestroy() or g_BonfireCtrl.m_CurTopicTime <= 0 then
			self:OnClose()
			return false
		end
		self.m_TopicTime:SetText("时间："..g_BonfireCtrl.m_CurTopicTime.."s")
		g_BonfireCtrl.m_CurTopicTime = g_BonfireCtrl.m_CurTopicTime - 1
		return true
	end
...
function CBonfireAnswerView.OnAudioRecord(self, oBtn, bPress)
    if bPress then
...
--开始录音
function CBonfireAnswerView.StartRecord(self, oBtn)
	-- 音量级减小
...
--结束录音
function CBonfireAnswerView.EndRecord(self)
	-- 音量恢复
...
function CBonfireAnswerView.OnAnswer(self, index)
	-- if g_BonfireCtrl.m_CurRemainTime > 0 then
	-- 	printc("当前未开始")
	-- 	return
	-- end
	if index == 0 and self.m_AnswerInput:GetText() == "" then
		g_NotifyCtrl:FloatMsg("请输入答案！")
		return
	else
		g_ChatCtrl:SendMsg(self.m_AnswerInput:GetText(), define.Channel.Org)
	end
	self.CurAnswerIdx = index
	g_BonfireCtrl:C2GSCampfireAnswer(g_BonfireCtrl.m_CurTopicInfo.id, index, self.m_AnswerInput:GetText())

end

function CBonfireAnswerView.OnClose(self)
	if self.m_DoneTimer then
		Utils.DelTimer(self.m_DoneTimer)
	end
	if self.m_RemainTimer then
