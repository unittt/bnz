...
function Color.RGBAToColor(rgbaColor)
    -- 得出十六进制的 R, G, B, A
    local hexR = string.sub(rgbaColor, 1, 2)
    local hexG = string.sub(rgbaColor, 3, 4)
    local hexB = string.sub(rgbaColor, 5, 6)
    local hexA = "FF"
    if #rgbaColor == 8 then
        hexA = string.sub(rgbaColor, 7, 8)
    end

    -- 把十六进制转为十进制 R, G, B, A
    local decR = tonumber("0x" .. hexR)
    local decG = tonumber("0x" .. hexG)
    local decB = tonumber("0x" .. hexB)
    local decA = tonumber("0x" .. hexA)

    local floatR = decR / 255
    local floatG = decG / 255
    local floatB = decB / 255
    local floatA = decA / 255

    return Color.New(floatR, floatG, floatB, floatA)
end
...
get.brown = function() return Color.New(0.5372549,0.3764705,0.3333333,1) end
get.turquoise = function() return Color.New(0, 0.5058823, 0.6705882, 1) end
get.orange = function() return Color.New(0.6509803, 0.3058823, 0, 1) end
