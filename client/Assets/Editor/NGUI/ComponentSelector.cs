...
public class ComponentSelector : ScriptableWizard
{
	...
	static public void Show<T> (OnSelectionCallback cb, bool needRefAtlas = false) where T : Object { Show<T>(cb, new string[] {".prefab"}, needRefAtlas); }

	/// <summary>
	/// Show the selection wizard.
	/// </summary>

    static public void Show<T>(OnSelectionCallback cb, string[] extensions, bool needRefAtlas = false) where T : Object
	{
        ...
        comp.needRefAtlas = needRefAtlas;
		...
		else
		{
            ...
            else if (typeof(T) == typeof(UIAtlas) && comp.needRefAtlas) //只显示Ref
            {
                for (int i = 0; i < comp.mObjects.Length; ++i)
                {
                    UIAtlas atlas = comp.mObjects[i] as UIAtlas;
		...
		if (mExtensions != null)
		{
			...
			for (int i = 0; i < paths.Length; ++i)
			{
				...
				else if (PrefabUtility.GetPrefabType(obj) == PrefabType.Prefab)
				{
                    ...
                    if (mType == typeof(UIAtlas) && needRefAtlas) //只显示Ref
                    {
                        UIAtlas atlas = t as UIAtlas;
