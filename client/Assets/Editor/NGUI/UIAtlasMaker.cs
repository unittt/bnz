...
public class UIAtlasMaker : EditorWindow
{
	...
	public List<string> forceDelNames = new List<string>();
	...
	public void OnSelectAtlas (Object obj)
	{
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (PackTextures(tex, sprites))
		{
            ...
            UIAtlasTools.SeperateOneAtlasRGBAlphaTexure(newPath);
			return true;
		}
		else
		{
    ...
    private static bool IsAtlasMatch(UIAtlas atlas)
    {
		if (atlas != null && atlas.texture != null && atlas.name != atlas.texture.name)
        {
            Debug.LogErrorFormat("Atlas 图集名字和贴图名字不一样  图集名{0} 贴图={1}", atlas.name, atlas.texture.name);
            return false;
        }
        return true;
    }
	...
	{
        if (!IsAtlasMatch(NGUISettings.atlas))
        {
            return;
        }
	...
	void OnGUI ()
	{
		...
		var forceDel = forceDelNames.Count > 0;
			...
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				if (delete || forceDel)
				{
					...
					for (int i = sprites.Count; i > 0; )
					{
						...
						if (mDelNames.Contains(ent.name) || forceDelNames.Contains(ent.name))
							sprites.RemoveAt(i);
					}
					UpdateAtlas(NGUISettings.atlas, sprites);
					mDelNames.Clear();
