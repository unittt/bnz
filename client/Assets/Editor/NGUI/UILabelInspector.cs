...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
        ...
        {
            GUI.changed = false;
            var styleList = UILabelStyleMenu.GetStyleNameList();
            styleList.Add("None");
            styleList.Sort();
            SerializedProperty prop = serializedObject.FindProperty("mTextStyle");
            
            string textStyle = NGUIEditorTools.DrawList("样式选择", styleList.ToArray(), prop.stringValue);
            if (GUILayout.Button("Apply", GUILayout.Width(60f)))
            {
                if (prop.stringValue != "None")
                {
                    mLabel.SetTextStyleData(UILabelStyleMenu.GetLabelByStyle(textStyle).GetTextStyleData());
		...
		EditorGUI.BeginDisabledGroup(!isValid);
        {
            ...
            NGUIEditorTools.DrawProperty("Text Bold", serializedObject, "mTextBold", GUILayout.Width(110f));
