...
public class UILabel : UIWidget
{
    ...
    [HideInInspector]
    [SerializeField]
    Overflow mOverflow = Overflow.ResizeFreely;
    [HideInInspector]
    [SerializeField]
    Material mMaterial;
    [HideInInspector]
    ...
    //获取textStyle
    public Dictionary<string, object> GetTextStyleData()
    {
    ...
    //设置textStyle
    public void SetTextStyleData(Dictionary<string, object> data)
    {
        this.mTrueTypeFont = (Font)data["mTrueTypeFont"];
        this.mFont = (UIFont)data["mFont"];
        this.mFontSize = (int)data["mFontSize"];
        this.mFontStyle = (FontStyle)data["mFontStyle"];
        this.mAlignment = (NGUIText.Alignment)data["mAlignment"];
        this.mEncoding = (bool)data["mEncoding"];
        this.mMaxLineCount = (int)data["mMaxLineCount"];
        this.mEffectStyle = (UILabel.Effect)data["mEffectStyle"];
        this.mEffectColor = (Color)data["mEffectColor"];
        this.mSymbols = (NGUIText.SymbolStyle)data["mSymbols"];
        this.mEffectDistance = (Vector2)data["mEffectDistance"];
        this.mOverflow = (UILabel.Overflow)data["mOverflow"];
        this.mMaterial = (Material)data["mMaterial"];
        this.mApplyGradient = (bool)data["mApplyGradient"];
        this.mGradientTop = (Color)data["mGradientTop"];
        this.mGradientBottom = (Color)data["mGradientBottom"];
        this.mSpacingX = (int)data["mSpacingX"];
        this.mSpacingY = (int)data["mSpacingY"];
        this.mUseFloatSpacing = (bool)data["mUseFloatSpacing"];
        this.mFloatSpacingX = (float)data["mFloatSpacingX"];
        this.mFloatSpacingY = (float)data["mFloatSpacingY"];
        this.mShrinkToFit = (bool)data["mShrinkToFit"];
        this.mMaxLineWidth = (int)data["mMaxLineWidth"];
        this.mMaxLineHeight = (int)data["mMaxLineHeight"];
        this.mLineWidth = (float)data["mLineWidth"];
        this.mMultiline = (bool)data["mMultiline"];
        this.color = (Color)data["mColor"];
        this.mTextBold = (bool)data["mTextBold"];
    }
        ...
        if (mFinalFontSize > 0)
        {
            ...
            for (int ps = mFinalFontSize; ps > 0; --ps)
            {
                ...
                else
                {
                    ...
                    NGUIText.emojiScale = mFont == null ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
    ...
    public void UpdateNGUIText()
    {
        ...
        NGUIText.textBold = mTextBold;

        NGUIText.emojiScale = mFont == null ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;

        NGUIText.fontScale = isDynamic ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
        NGUIText.maxEmojiHeight = 0f;
        if (mFont != null)
        {
    ...
    private void UpdateEmoji()
    {
        ...
        if (mEmojiTex == null)
        {
            ...
            if (emojiChild != null)
            {
                ...
                mEmojiTex.transform.localPosition = Vector3.zero;
            }
            else
            {
            ...
            mEmojiTex.updateAnchors = AnchorUpdate.OnEnable;
