...
public class UIInput : MonoBehaviour
{
    ...
    //添加foucs通知
    public List<EventDelegate> onFocusChange = new List<EventDelegate>();
...
#if MOBILE_ANDROID
		if (isSelected && UnityEditDialog.IsProcessing(this) && mCached != value)
		{
			mCached = value;
		}
#elif MOBILE
...
#if MOBILE_ANDROID
            int result;
            if (UnityEditDialog.IsProcessing(this))
            {
	...
	public int selectionStart
	{
		get
		{
            ...
            {
                result = (this.isSelected ? this.mSelectionStart : this.value.Length);
            }
            return result;
#elif MOBILE
	...
	public int selectionEnd
	{
		get
        {
            ...
            {
                result = (this.isSelected ? this.mSelectionEnd : this.value.Length);
            }
            return result;
#elif MOBILE
			if (mKeyboard != null && !inputShouldBeHidden) return value.Length;
	...
	public string Validate (string val)
	{
		...
		for (int i = 0; i < val.Length; ++i)
		{
			...
			if (c != 0)
            {
                if (characterLimit > 0)
                 {
                    addCount += GetCharLength(c);
                    if (addCount > characterLimit)
                    {
                        break;
                    }  
                 }
	...
	protected void OnSelectEvent ()
	{
        ...
        if (EventDelegate.IsValid(this.onFocusChange))
        {
            EventDelegate.Execute(this.onFocusChange);
        }
	}
    ...
    protected virtual void Update ()
    {
		...
		if (mSelectMe != -1 && mSelectMe != Time.frameCount)
		{
            ...
            if (!UnityEditDialog.IsProcessing(this))
            {
                ...
                UnityEditTextStyle unityEditTextStyle = new UnityEditTextStyle();
                string text;
                if (this.inputType == UIInput.InputType.Password)
                {
                    unityEditTextStyle.inputMode = UIInput.KeyboardType.Default;
                    text = this.mValue;
                    this.mSelectionStart = this.mSelectionEnd;
                }
                else
                {
                    unityEditTextStyle.inputMode = this.keyboardType;
                    text = this.mValue;
                    this.mSelectionStart = this.mSelectionEnd;
                }
                //Vector3 vector = UICamera.current.cachedCamera.WorldToScreenPoint(this.label.worldCorners[0]);
                //Vector3 vector2 = UICamera.current.cachedCamera.WorldToScreenPoint(this.label.worldCorners[2]);

                Vector3 vector = (UICamera.current != null && UICamera.current.cachedCamera != null) ? UICamera.current.cachedCamera.WorldToScreenPoint(label.worldCorners[0]) :label.worldCorners[0];
                Vector3 vector2 = (UICamera.current != null && UICamera.current.cachedCamera != null) ? UICamera.current.cachedCamera.WorldToScreenPoint(label.worldCorners[2]) :label.worldCorners[2];
                unityEditTextStyle.left = 300;
                unityEditTextStyle.top = 300;
                unityEditTextStyle.width = 300;
                unityEditTextStyle.height = 300;
                unityEditTextStyle.maxLength = this.characterLimit;
                unityEditTextStyle.textSize = (float)(this.label.fontSize * Screen.height / UIRoot.list[0].manualHeight);
                //unityEditTextStyle.textColor = this.label.color;
                unityEditTextStyle.alignment = NGUIText.Alignment.Left; //this.label.alignment;
                unityEditTextStyle.inputMode = this.keyboardType;
                unityEditTextStyle.inputFlag = this.inputType;
                unityEditTextStyle.inputReturn = this.onReturnKey;
                //UnityEditDialog.onInputCover = OnInputCover;
                UnityEditDialog.onInputTextChanged = this.OnInputTextChanged;
                UnityEditDialog.onInputReturn = this.OnInputReturn;
                UnityEditDialog.onInputFinish = this.OnInputFinish;
                UnityEditDialog.onInputValidate = this.Validate;
                UnityEditDialog.Show(this, text, unityEditTextStyle);
            }
        ...
        if (UnityEditDialog.IsProcessing(this))
        {
            ...
            {
                bool flag = this.onReturnKey == UIInput.OnReturnKey.NewLine || (this.onReturnKey == UIInput.OnReturnKey.Default && this.label.multiLine && this.label.overflowMethod != UILabel.Overflow.ClampContent && this.validation == UIInput.Validation.None);
                if (flag)
                {
                    this.Insert("\n");
                }
...
#endif // MOBILE
        {
            ...
            //add by llw
            if (!string.IsNullOrEmpty(ime) && !ime.Equals(inputString) && ime.Length == 1)
            {
                Insert("");
            }
            inputString = ime;
	...
	protected virtual void Insert (string text)
	{
		...
		int rl = right.Length;
        int leftCount = GetStringLength(left);
        int rightCount = GetStringLength(right);
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            //中文长度=2 英文=1
            addCount += GetCharLength(c);
            if (characterLimit > 0 && leftCount + rightCount + addCount> characterLimit)
                break;
	...
	protected string GetRightText ()
	{
        ...
        string v = label.text;
        return (string.IsNullOrEmpty(v) || max >= v.Length) ? "" : v.Substring(max);
	}
	...
	protected string GetSelection ()
	{
		...
		else
		{
            ...
            //add by llw
            min = Mathf.Min(min, mValue.Length);
            max = Mathf.Min(max, mValue.Length);
	...
	public void UpdateLabel ()
	{
		if (label != null)
		{
			...
			else
			{
                ...
                int selPos = selected ? Mathf.Min(processed.Length, mSelectionStart) : 0;
    ...
    private void OnInputTextChanged(string text)
    {
        if (this.mCached != text)
        {
            ...
            if (this.onChange != null)
            {
                ...
                {
                    this.onChange[i].Execute();
                }
            }
        }
    }

    private void OnInputReturn()
    {
        if (this.onSubmit != null)
        {
            for (int i = 0; i < this.onSubmit.Count; i++)
            {
                this.onSubmit[i].Execute();
            }
        }
    }
    ...
    private void OnInputFinish()
    {
        ...
        this.mCached = "";
        UnityEditDialog.onInputCover = null;
        UnityEditDialog.onInputTextChanged = null;
        UnityEditDialog.onInputReturn = null;
        UnityEditDialog.onInputFinish = null;
        UnityEditDialog.onInputValidate = null;
    }
