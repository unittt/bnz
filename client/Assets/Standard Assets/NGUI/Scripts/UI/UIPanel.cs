...
public class UIPanel : UIRect
{
    ...
    public int uiEffectDrawCallCount = 0;
    ...
    public static void RebuildAllPanel()
    {
        for(int i = 0; i < list.Count; i++)
        {
            list[i].mRebuild = true;
        }
    }
    ...
    protected void OnDestroy()
    {
        ...
        {
            if (widgets[i].geometry != null)
            {
                UIGeometry.Recycle(widgets[i].geometry);
                widgets[i].geometry = null;
            }
        }
    }
...
#endif
		{
			...
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
				...
				if (p.renderQueue == RenderQueue.Automatic)
				{
					...
					p.UpdateDrawCalls();
                    rq += p.drawCalls.Count + p.uiEffectDrawCallCount;
				}
