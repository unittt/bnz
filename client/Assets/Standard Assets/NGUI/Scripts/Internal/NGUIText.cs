...
static public class NGUIText
{
    ...
    static public float maxEmojiHeight = 0f;
    static public float emojiScale = 1f;

    static public bool textBold = false;

    static private float trueLineHeight
    {
        get {
            float emojiHeightWithSpacing = maxEmojiHeight + spacingY*fontScale;
            float h = emojiHeightWithSpacing > finalLineHeight ? emojiHeightWithSpacing :finalLineHeight;
            maxEmojiHeight = 0f;
            return h;
        }
    }
		...
		ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike, ref bool ignoreColor)
	{
        ...
        if (ColorShortText.Instance && text[index] == '#' && (index + 1) < length)
        {
            ...
            else if ( ColorShortText.Instance.colors.ContainsKey(nextChar))
            {
                if (colors != null)
                {
                    colors.Add(ColorShortText.Instance.colors[nextChar]);
                }
                index += 2;
                return true;
            }
	...
	static public Vector2 CalculatePrintedSize (string text)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				{
					float w = finalSpacingX + symbol.advance * emojiScale;
                    ...
                    float emojiHeight = symbol.printHeight * emojiScale;
                    maxEmojiHeight = emojiHeight > maxEmojiHeight ? emojiHeight : maxEmojiHeight;
				}
			}
            v.x = ((x > maxX) ? x - spacingX : maxX);
            v.y = (y + trueLineHeight);
		}
		return v;
	}
	...
	static public int CalculateOffsetToFit (string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
			...
			{
				mSizes.Add(finalSpacingX + symbol.advance * emojiScale);
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false)
	{
		...
		for (; offset < textLength; ++offset)
		{
			...
			else glyphWidth = finalSpacingX + symbol.advance * emojiScale;
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols,UIGeometry emoji)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol != null)
			{
                ...
                v1x = v0x + symbol.printWidth * emojiScale;

				v1y = -(y + symbol.offsetY * fontScale);
                float emojiHeight = symbol.printHeight * emojiScale;
				v0y = v1y - emojiHeight;
				// Doesn't fit? Move down to the next line
				if (Mathf.RoundToInt(x + symbol.advance * emojiScale) > regionWidth)
				{
					if (x == 0f) return;
				...
				x += finalSpacingX + symbol.advance * emojiScale;
				i += symbol.length - 1;
				prev = 0;
	...
	static public void PrintApproximateCharacterPositions (string text, BetterList<Vector3> verts, BetterList<int> indices)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			{
				float w = symbol.advance * emojiScale + finalSpacingX;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol == null)
			{
				...
				if (gw != 0f)
				{
                    ...
                    w = x + w + finalSpacingX <= regionWidth ? w + finalSpacingX : w;
                    indices.Add(i);
			...
			{
                float w = symbol.advance * emojiScale;
                ...
                maxEmojiHeight = emojiHeight > maxEmojiHeight ? emojiHeight : maxEmojiHeight;
				indices.Add(i);
                if (emojiHeight > fullSize)
                {
                    verts.Add(new Vector3(x, -y - emojiHeight));
                }
                else
                {
                    verts.Add(new Vector3(x, -y - fullSize));
	...
	static public void PrintCaretAndSelection (string text, int start, int end, BetterList<Vector3> caret, BetterList<Vector3> highlight)
	{
		...
		for (; index < textLength; ++index)
		{
			...
			float w = (symbol != null) ? symbol.advance * emojiScale : GetGlyphWidth(ch, prev);
