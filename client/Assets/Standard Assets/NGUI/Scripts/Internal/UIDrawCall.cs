...
public class UIDrawCall : MonoBehaviour
{
    ...
    public static HashSet<string> activeTextureNameSet = new HashSet<string>();
    public static HashSet<string> usedTextureNameSet = new HashSet<string>();
    public static HashSet<string> unusedTextureNameSet = new HashSet<string>();

    public static HashSet<string> GetUnnsedTextureNameSet()
    {
        unusedTextureNameSet.Clear();
        activeTextureNameSet.Clear();

        for (int i = 0; i < activeList.size; i++)
        {
            if (activeList[i] != null && !string.IsNullOrEmpty(activeList[i].textureName))
            {
                activeTextureNameSet.Add(activeList[i].textureName);
            }
        }

        foreach (string name in usedTextureNameSet)
        {
            if (!activeTextureNameSet.Contains(name))
            {
                unusedTextureNameSet.Add(name);
            }
        }
        usedTextureNameSet.Clear();
        return unusedTextureNameSet;
    }
	...
	void CreateMaterial ()
	{
		...
		{
			mTextureClip = true;
			shader = ShaderHelper.Find("Hidden/" + shaderName + textureClip);

            if (mClipCount > 1)
            {
                string name = string.Format("Hidden/{0}{1} {2}", shaderName, textureClip, mClipCount);
                if(ShaderHelper.Find(name))
                {
                    shader = ShaderHelper.Find(name);
		...
		{
			shader = ShaderHelper.Find("Hidden/" + shaderName + " " + mClipCount);
			if (shader == null) shader = ShaderHelper.Find(shaderName + " " + mClipCount);

			// Legacy functionality
			if (shader == null && mClipCount == 1)
			{
				mLegacyShader = true;
				shader = ShaderHelper.Find(shaderName + soft);
			}
		}
		else shader = ShaderHelper.Find(shaderName);

		// Always fallback to the default shader
		if (shader == null) shader = ShaderHelper.Find("Unlit/Transparent Colored");
	...
	static UIDrawCall Create (string name, UIPanel pan, Material mat, Texture tex, Shader shader)
	{
        ...
        {
            usedTextureNameSet.Add(tex.name);
        }
