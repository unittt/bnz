...
public class UIWrapContent : MonoBehaviour
{
    ...
    public int reuqiredItemCnt
    {
        get
        {
            var region = scrollView.panel.baseClipRegion;
            int i = Mathf.CeilToInt(horizontal ? region.z / itemSize : region.w / itemSize) + 2;
            return Mathf.Min(Mathf.Abs(minIndex - 1), i);
        }
    }
    ...
    public void MoveRelative(Vector3 v)
    {
        ...
        if (mHorizontal)
        {
            v.x = Mathf.Floor(v.x / itemSize) * itemSize;
        }
        else
        {
            v.y = Mathf.Floor(v.y / itemSize) * itemSize;
        }
        for (int i = 0, imax = mChildren.Count; i < imax; ++i)
        {
            Transform t = mChildren[i];
            ...
            if (mHorizontal)
            {
                pos.x = (itemSize * (i - 1) + v.x) * -1;
            }
            else
            {
                pos.y = (itemSize * (i - 1) + v.y) * -1;
            }

            t.localPosition = pos;
            UpdateItem(t, i);
        }
        scrollView.MoveRelative(tV);
        mFirstTime = false;
    }
