...
public class EasyTouch : MonoBehaviour {
	...
	// input
	public EasyTouchInput input = new EasyTouchInput();
	#endregion
	...
	void LateUpdate(){
		...
		if (_currentGestures.Count>1)
        {
            ...
            _currentGestures[0] = NewGesture();
		}

        CacheGesture(_currentGesture);
		_currentGesture = _currentGestures[0];

	}

    public Stack<Gesture> cacheGestureStack = new Stack<Gesture>();

    public Gesture NewGesture()
    {
        if (cacheGestureStack.Count > 0)
        {
            return cacheGestureStack.Pop();
        }
        else
        {
            return new Gesture();
    ...
    public void CacheGesture(Gesture gesture)
    {
        if (gesture != null && cacheGestureStack.Count < 2)
        {
            cacheGestureStack.Push(gesture);
        }

    }

	public bool isRealTouch;	
 
	void UpdateTouches(bool realTouch, int touchCount){
        isRealTouch = realTouch;
    ...
    public static Vector2 GetFingerPosition(int fingerId)
    {
        Finger finger = EasyTouch.instance.GetFinger(fingerId);
        Vector2 result;
        if (finger != null)
        {
            result = finger.position;
        }
        else
        {
            result = Vector2.zero;
